...
export async function ensureOutboundUsageChargesTable() {
  const sql = getSql();
  await sql.unsafe(`
    CREATE TABLE IF NOT EXISTS outbound_usage_charges (
      ...
      CONSTRAINT outbound_usage_charges_wallet_check CHECK (
        wallet_source IN ('main')
      )
    )
  `);

  await sql.unsafe(`
    CREATE INDEX IF NOT EXISTS outbound_usage_charges_sub_idx
    ON outbound_usage_charges (subscription_id)
  `);

  await sql.unsafe(`
    CREATE INDEX IF NOT EXISTS outbound_usage_charges_user_idx
    ON outbound_usage_charges (user_row_id, date_created DESC)
  `);
}

export async function createOutboundUsageCharge(txOrSql, input) {
  const sql = txOrSql;
  const [row] = await sql`
    INSERT INTO outbound_usage_charges (
...
export async function listOutboundUsageChargesForUser(telegramUserId, limit = 100) {
  const sql = getSql();
  return sql`
    SELECT
      c.*,
...
export async function listOutboundVolumePurchasesForUser(telegramUserId, limit = 50) {
  const sql = getSql();
  return sql`
    SELECT
      s.id,
      s.client_username,
      s.volume_gb,
      s.purchase_amount_irt,
      s.created_at,
      s.service_type
    FROM user_panel_subscriptions s
    JOIN users u ON u.id = s.user_row_id
    WHERE u.user_id = ${Number(telegramUserId)}
      AND s.service_type = 'outbound_volume'
      AND s.purchase_amount_irt IS NOT NULL
...
export async function getAdminOutboundUsageInvoices({ range = "week", limit = 50, offset = 0 } = {}) {
  const sql = getSql();

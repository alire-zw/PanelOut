...
import { activatePanelUsage } from '../lib/panelApi'
import type { PanelCredentialsState } from '../types/panel'
import '../styles/shop-rise.css'
import './PanelFlow.css'

export function PanelUsageUsernamePage() {
  useEnsureUser()
  ...
  const handleSubmit = async (event?: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'خطا در فعال‌سازی پنل')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="panel-flow">
      <div className="shop-rise" style={{ '--rise-index': 0 } as CSSProperties}>
        <PageHeader title="نام کاربری پنل" onBack={() => navigate('/panel/usage')} />
      </div>
      ...
      <footer className="panel-flow__footer shop-rise" style={{ '--rise-index': 2 } as CSSProperties}>
        ...
        >
          {busy ? 'در حال فعال‌سازی…' : 'فعال‌سازی و دریافت اطلاعات'}
        </button>
      </footer>
    </div>
  )

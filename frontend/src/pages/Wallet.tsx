...
function formatTransactionAmount(amount: number, status: WalletTransactionStatus): string {
...
function TransactionRow({
  ...
  return (
    <div
      className={`wallet__transaction${isClickable ? ' wallet__transaction--clickable' : ''}`}
...
export function WalletPage() {
  ...
  const { user, error: userError, isLoading: isUserLoading } = useEnsureUser()
  const balance = user ? balanceToToman(user.balance) : 0
  const balanceError = userError
  const isBalanceLoading = isUserLoading && !user
  ...
  const handleTransactionClick = useCallback(
    (transaction: WalletTransaction) => {
      ...
      if (transaction.status === 'pending' && transaction.type === 'deposit') {
        ...
        setNotification({
          show: true,
          message: 'ادامه پرداخت به‌زودی فعال می‌شود',
          type: 'info',
        })
        return
      }
    ...
    <div className="wallet">
      ...
      <div className="wallet__top">
        <div className="shop-rise" style={{ '--rise-index': 0 } as CSSProperties}>
          <PageHeader title="کیف پول" onBack={() => navigate('/')} />
        </div>
          ...
          <div className="wallet__balance">
            {isBalanceLoading ? (
              ...
              <div className="wallet__balance-error">
                <span className="wallet__balance-error-text">{balanceError}</span>
              </div>
            ) : (
              <>
        ...
        <h3 className="wallet__section-title shop-rise" style={{ '--rise-index': 3 } as CSSProperties}>
          تراکنش‌ها
